...
function getPluginsForGithubClass(pluginClasses, blob) {
  // TODO add tests for all this
  return flattenAndCompact(pluginClasses.map((PluginClass) => {
    const plugin = new PluginClass();
    if (!plugin.getGithubClass) {
      return [];
    }
    if (blob.el.classList.contains(plugin.getGithubClass())) {
      return [plugin];
    }
    return [];
  }));
...
export default class {
  ...
  get(blob) {
    ...
    return getPluginsForGithubClass(this._pluginClasses, blob);
  }
}
