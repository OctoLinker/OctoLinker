...
function javascriptFileHelper({ path, target }) {
  const list = [];
...
export function javascriptFile({ path, target }) {
  ...
  const firstDir = normalize(target).split('/')[0];

  if (firstDir === 'dist') {
    targets.push(target.replace(firstDir, 'lib'));
    targets.push(target.replace(firstDir, 'src'));
  } else if (firstDir === 'lib') {
    targets.push(target.replace(firstDir, 'src'));
  }

  return concatMap(targets, targ => javascriptFileHelper({ path, target: targ }));
}
